...
if __name__ == "__main__":
    ...
    for Img_file in [Img_files[8]]:       
        ...
        print(compute_mse_loss(alpha_samp, Alpha, Trimap))
        print(compute_mse_loss(alpha_weight, Alpha, Trimap))
        save_path="D:\\BangweiYe\\matting\\output\\training2.0\\"
    ...
    '''
    save_path="D:\\BangweiYe\\matting\\output\\testing\\"
    cv2.imwrite( os.path.join(save_path+"samp\\Trimap3",Img_file) , alpha_samp)
    cv2.waitKey(0)
    show(alpha_samp)
    cv2.imwrite( os.path.join(save_path+"weight\\Trimap3",Img_file) , alpha_weight)
    cv2.waitKey(0)
    show(alpha_weight)
    Trimap = cv2.imread(Trimap_dir, cv2.IMREAD_GRAYSCALE)    
    Alpha=cv2.imread(os.path.join(path+"gt_training_lowres", Img_file), cv2.IMREAD_GRAYSCALE)
    ...
    compute_mse_loss(alpha_samp*255, Alpha, Trimap)
    compute_mse_loss(alpha_weight*255, Alpha, Trimap)
    '''
