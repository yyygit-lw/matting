# -*- coding: utf-8 -*-
"""
Created on Sat May 13 15:28:26 2023
...
def get_Imgfeat(Img):
    ...
    return   np.concatenate((Img_lab,Img_hsv,Img_id,Img_lap,Img_lbp,Img_lbp_unfrevolve,Img_grad,Img_hog),axis=2)#np.concatenate((Img_gray,Img_lab,Img_hsv,Img_id,Img_lbp,Img_var,lab_grad,Img_hog),axis=2)
...
def get_leaf(Img,clf,Trimap):   
    foreground_ind,Trimap=pre_proces4dk(Trimap,thr_C=5,thr_E=7,region=255)
    prior=Trimap/255
...
def matting(k):
    ...
    if max_dF<0.01:
        ...
        cost_mat=cost_mat+(Fdist_mat+Bdist_mat)*0.5
        ...
        Fcost,Bcost=Fcost+Fdist,Bcost+Bdist
...
def pre_proces4dk(Trimap,thr_C,thr_E,region=None):
                            ...
                            if dist<=thr_E:
                                for (x,y) in [(i+m,j+n),(i+m,j-n),(i-m,j+n),(i-m,j-n)]:
                                    ...
                                    if 0<=x<rows and 0<=y<cols and Trimap_mid2[x,y]==region and sum((Img[x,y]-Img[i,j])**2)<=(thr_C-dist)**2:#if 0<=x<rows and 0<=y<cols and Trimap_mid2[x,y]!=128 and sum((Img[x,y]-Img[i,j])**2)<=(thr_C-dist)**2:
                                        ...
                                        foreground_ind[i,j]=True
        return foreground_ind,Trimap_mid2
...
def priormatting(Img,Trimap_real,Img_file):
    ...
    #show(Img)
    Trimap=pre_proces4dk(Trimap_real,thr_C=3,thr_E=4,region=0)
    #Trimap=pre_proces4dk(Trimap,thr_C=5,thr_E=7,region=255)
    #Trimap=pre_proces(Trimap,thr_C=2,thr_E=2,region=0)#Trimap=pre_proces(Trimap,thr_C=2,thr_E=2,region=0)    
    ...
    for k in range(n_test):
        alpha_samp[Imgxy_testing[k][-2],Imgxy_testing[k][-1]]=pred_seq[k]
...
if __name__ == "__main__":
    ...
    for Trimap_cls in ["Trimap2"]:#,
        ...
        alpha_sampseq=alpha_samp[Trimap_real==128]
        '''
