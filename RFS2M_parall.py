# -*- coding: utf-8 -*-
"""
Created on Fri Oct 14 14:12:16 2022
...
def matting(k):
    ...
    if np.sum(nsamB>0)==0 or (dF>=thup and dB<=thdn):
        ...
        if dF>=thup:
            ...
            ints=int(dF/thdn2)
            Fset=list(np.where((ints*thdn2<nsamF)&(nsamF<=dF))[0])
            for dg in range(0,ints):
                try:
                    temp=random.sample(list(np.where((dg*thdn2<nsamF)&(nsamF<=(dg+1)*thdn2))[0]),num_each)
                except:
                    temp=list(np.where((dg*thdn2<nsamF)&(nsamF<=(dg+1)*thdn2))[0])
                Fset=Fset+temp
            if len(Fset)>n_smp:
        ...
        if dB>=thup:
            ...
            ints=int(dB/thdn2)
            Bset=list(np.where((ints*thdn2<nsamB)&(nsamB<=dB))[0])
            for dg in range(0,ints):
                try:
                    temp=random.sample(list(np.where((dg*thdn2<nsamB)&(nsamB<=(dg+1)*thdn2))[0]),num_each)
                except:
                    temp=list(np.where((dg*thdn2<nsamB)&(nsamB<=(dg+1)*thdn2))[0])
                Bset=Bset+temp
            if len(Bset)>n_smp:
                Bset=random.sample(Bset,n_smp)
        Fpixy_seq,Bpixy_seq=Imgxy_F[Fset],Imgxy_B[Bset]
        pix,xy_pix,Mincost=pixy[:3],pixy[-2:],MAX_INT32
        Fdist=[ np.sqrt(np.sum((xy_pix- item)**2)) for item in (Fpixy_seq.T[-2:]).T]
        Bdist=[ np.sqrt(np.sum((xy_pix- item)**2)) for item in (Bpixy_seq.T[-2:]).T]
        ...
        for fpixy in Fpixy_seq:
            ...
            for bpixy in Bpixy_seq:   
                ...
                tmp1,tmp2 = pix-b,f-b
                tmp3,tmp4 = np.sum(tmp1 * tmp2),np.sum(tmp2 * tmp2)
                alpha=1 if tmp4 == 0 else min(max(tmp3 / tmp4, 0.0),1)
                if -1*th<best_alpha-alpha<th:
                    C_dist = pix - (alpha * f + (1-alpha) * b)                           
                    try:
                        cost=np.sqrt(np.sum(C_dist*C_dist)/tmp4)+(S_distF+S_distB)*0.75
                    except:
                        cost=MAX_INT32
                    if cost<Mincost:
                        Mincost=cost
...
def priormatting(Img,Trimap,Img_file):
    ...
    show(prior)
    #print(compute_mse_loss(prior*255, Alpha, Trimap_real))
    thup,thdn,thdn2=0.5,0.05,0.02
    n_smp,zeros,num_each=300,np.zeros(shape=(1,200),dtype=(int)),60 #100为佳
...
if __name__ == "__main__":
    ...
    print("matt:"+Img_file)
    alpha_samp,alpha_weight=priormatting(Img,Trimap,Img_file)#Img, prior,Trimap_dir,Alpha,Img_file
    save_path="D:\\BangweiYe\\matting\\output\\testing2.0\\"
