# -*- coding: utf-8 -*-
"""
Created on Fri Nov 11 17:37:31 2022
...
def get_Imgfeat(Img):
    ...
    return np.concatenate((Img_gray,Img_lab,Img_hsv,Img_lbp,Img_var,lab_grad,Img_hog),axis=2)#np.concatenate((Img_gray,Img_lab,Img_hsv,Img_id,Img_lbp,Img_var,lab_grad,Img_hog),axis=2)
...
def get_FBset(dsam_obj,max_d):
    ...
    gap=dsam_obj[obj_set[0]]-low_bd
    if gap<=0:
        return np.where(dsam_obj>low_bd)[0]
    else:
        dgs=[1,2/3,1/3,0]
        for t in range(3):
            try:
...
def matting(k):
    ...
    if max_dF<0.01:
        ...
        nlzFdist,nlzBdist=nlzFdist.reshape(-1,1)@e_B,e_F@nlzBdist.reshape(1,-1)
        cost_mat=cost_mat+(nlzFdist+nlzBdist)*0.75
        Mincost=np.min(cost_mat)        
        best_alpha=alpha_matrav[np.argmin(cost_mat)]
            
        '''
        best_alpha,pix,xy_pix,Mincost=prior[i][j],pixy[:3],pixy[-2:],MAX_INT32
        xy_dF,xy_dB=xy_pix-Fpixy_seq[:,-2:],xy_pix-Bpixy_seq[:,-2:]
        Fdist,Bdist=np.linalg.norm(xy_dF,axis=1),np.linalg.norm(xy_dB,axis=1)
        Fdist_mean,Bdist_mean=np.mean(Fdist),np.mean(Bdist)
        nlzFdist,nlzBdist=Fdist/Fdist_mean,Bdist/Bdist_mean
        
        for p in range(l_F):
            f,S_distF=F[p][:3],nlzFdist[p]
            th=0.15 if 0.2<=best_alpha<=0.8 else 0.1
            for q in range(l_B):                 
                b,S_distB=Bpixy_seq[q][:3],nlzBdist[q]
                alpha=alpha_mat[p,q]
                if -1*th<best_alpha-alpha<th:
                    C_dist = pix - (alpha * f + (1-alpha) * b)                           
                    cost=np.linalg.norm(C_dist)/alpha_normsq[p,q]+(S_distF+S_distB)*0.75
                    if cost<Mincost:
                        Mincost=cost
                        best_alpha=alpha
        '''  
        ...
        Fcost,Bcost=Fcost+nlzFdist[:,0]*0.75+0.75*Fdist/Bdist_mean,Bcost+nlzBdist[0,:]*0.75+0.75*Bdist/Fdist_mean
        MinFcost,MinBcost=np.min(Fcost),np.min(Bcost)
        if Mincost>min(MinFcost,MinBcost):
            if MinFcost<MinBcost and prior[i][j]>0.5:
                best_alpha=1  
            elif MinFcost>MinBcost and prior[i][j]<0.5:
                best_alpha=0
...
def priormatting(Img,Trimap_real,Img_file):
    ...
    n_smp,zeros,num_each=300,np.zeros(shape=(1,200),dtype=(int)),[65,20,10,5]#[50,70,80,100]# #60为佳
    ...
    pred_seq=Parallel(n_jobs=-1, verbose=0, backend='threading')(delayed(matting)(k) for k in range(n_test))
