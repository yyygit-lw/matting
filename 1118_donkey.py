# -*- coding: utf-8 -*-
"""
Created on Sun Nov 13 12:51:03 2022
...
def get_Imgfeat(Img):
    ...
    return   np.concatenate((Img_lab,Img_hsv,Img_id,Img_lbp,Img_grad),axis=2)#np.concatenate((Img_gray,Img_lab,Img_hsv,Img_id,Img_lbp,Img_var,lab_grad,Img_hog),axis=2)
...
def get_FBset(dsam_obj,max_d):
    ...
    if gap<=0:
        ...
        if len(obj_set)>n_smp:
            obj_set=obj_set[n_smp]
...
def matting(k):
    ...
    if max_dF<0.01:
        ...
        Fcost,Bcost=Fcost+1.5*Fdist,Bcost+1.5*Bdist
        MinFcost,MinBcost=np.min(Fcost),np.min(Bcost)
        
        if MinFcost<Mincost and prior[i][j]>0.5: #
            best_alpha=0 if MinBcost<MinFcost else 1
        elif MinBcost<Mincost and prior[i][j]<0.5: #
            best_alpha=1 if MinFcost<MinBcost else 0
        return (best_alpha,-1,-1)
        '''
        if (best_alpha<0.5 and MinFcost<MinBcost) or (best_alpha>0.5 and MinBcost<MinFcost):
            return (1-best_alpha,i,j) 
        else:
            return (best_alpha,i,j)
        '''''''
        Img_sm=np.copy(Img)
...
def priormatting(Img,Trimap_real,Img_file):
    global zerones,unknown_seq,testing_leaf,n_test,n_tree,XF_leaf,XB_leaf,zeros,prior,thup,thdn,thdn2,n_smp,Imgxy_testing,Imgxy_F,Imgxy_B,num_each     
    #Trimap_real = cv2.imread(Trimap_dir, cv2.IMREAD_GRAYSCALE)    
    #Alpha=cv2.imread(os.path.join(path+"gt_training_lowres", Img_file), cv2.IMREAD_GRAYSCALE)    
    #show(ps_Alpha)
    Trimap=pre_proces(Trimap_real,thr_C=3,thr_E=4,region=0)
    Trimap=pre_proces(Trimap,thr_C=5,thr_E=7,region=255)
    ...
    #show((Trimap!=128) &(Trimap!=ps_Alpha))
    show(Trimap)
    #cv2.imwrite("ttry.png",Trimap)
    
    #Trimap=np.copy(Trimap_real)
    clf = RandomForestClassifier(n_estimators=200,criterion="entropy",max_features=1)#
    prior,XF_leaf,XB_leaf,testing_leaf,Imgxy_F, Imgxy_B, Imgxy_testing=get_leaf(Img,clf,Trimap)
    show(prior)
    #compute_mse_loss(prior*255, ps_Alpha,Trimap_real)
    
    #cv2.imwrite("ttry2.png",prior*255)
    ...
    zerones=np.zeros_like(Trimap)
    pred_seq=Parallel(n_jobs=-1)(delayed(matting)(k) for k in range(n_test))
    #pred_seq=[matting(k) for k in range(11957,12957)]
    for k in range(n_test):
        alpha_samp[unknown_seq[k]]=pred_seq[k][0]
        if pred_seq[k][1]!=-1:
            zerones[pred_seq[k][1],pred_seq[k][2]]=255
...
if __name__ == "__main__":
    ...
    for Trimap_cls in ["Trimap2"]:#,
        ...
        show(ttry)
        show(zerones)
        show((ttry==True) & (zerones==255))
        cv2.imwrite("ttry.png",zerones)
