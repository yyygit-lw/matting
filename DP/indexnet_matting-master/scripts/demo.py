...
from hlmobilenetv2 import hlmobilenetv2#自建
...
STRIDE = 32
RESTORE_FROM = 'D:/BangweiYe/matting/DP/indexnet_matting-master/pretrained/indexnet_matting.pth.tar'
RESULT_DIR = 'D:/BangweiYe/matting/DP/indexnet_matting-master/examples/mattes'
...
def image_alignment(x, output_stride, odd=False):
    imsize = np.asarray(x.shape[:2], dtype=float)
    if odd:
...
if __name__ == "__main__":
    ...
    '''
    path= 'D:/BangweiYe/matting/DP/indexnet_matting-master/examples/'
    image_path,trimap_path=path+'images/',path+'trimaps/'
    image_files= os.listdir(image_path)
    for image_file in image_files:
        image, trimap=os.path.join(image_path, image_file) ,os.path.join(trimap_path, image_file) 
        print(image_file)
        inference(image, trimap) 
