function alpha = PDMS_tiaoshi(img_name,Trimap_name,test,i,j)
...
for n = 1:length(U_ind)
    if and(x(U_ind(n))==j , y(U_ind(n))==i)
        ttry=n;
    end
end
n=ttry;
...
if (test==3)
    ...
    pareto_F_bw = FDMOt([spatial_cost_FU,color_cost_FU]);%
    pareto_B_bw = FDMOt([spatial_cost_BU,color_cost_BU]);%    
end
%%%
indf=find(pareto_F_bw==1);
indb=find(pareto_B_bw==1);
...
[bestf_ind,bestb_ind,alpha] = EvaluateSamples(pareto_F_bw,pareto_B_bw,...
    F_pareto_color,B_pareto_color,F_pareto_dist,B_pareto_dist,U_color);

xyf=[y(F_ind(indf(bestf_ind)))-1,x(F_ind(indf(bestf_ind)))-1]
xyb=[y(B_ind(indb(bestb_ind)))-1,x(B_ind(indb(bestb_ind)))-1]
alpha
